...
const SingleWeatherStationTable: React.FC<{ selectedStation: Station }> = ({
  ...
  const createTableForDate = () => {
    ...
    for (let i = 0; i < nextElevenDays.length; i++) {
      ...
      const filterDataByDate = selectedStation.forecast.filter((forecast) => {
        return (
          moment(forecast.ftime, 'YYYY-MM-DD HH:mm:ss').format('DDD') ===
          ...
          >
            ...
            <TableRewind.Tbody>
              {filterDataByDate.map((singleForecast) => (
                <TableRewind.Tr key={singleForecast.ftime.toString()}>
                  <TableRewind.Td style={{ width: '25%' }} align="center">
                    {moment(singleForecast.ftime, 'YYYY-MM-DD HH:mm:ss').format(
                      'HH:mm',
                    )}
                  </TableRewind.Td>
                  <TableRewind.Td style={{ width: '40%' }} align="center">
                    {singleForecast.W}
                  </TableRewind.Td>
