...
const DropDownWeatherStationSelector: React.FC<{
  ...
  const updateCheckStatus = (index: number) => {
    setAllWeatherStationCheckboxes(
      allWeatherStationCheckboxes.map((weatherStationCheckbox, currentIndex) =>
        currentIndex === index
          ? {
  ...
  return (
    <>
      ...
      >
        ...
        >
          <h1>Veðurstöðvar</h1>
          {allWeatherStationCheckboxes.map((weatherStationCheckbox, index) => (
            <Checkbox
          ...
          <Button onClick={() => filterData(allWeatherStationCheckboxes)}>
            Sía
          </Button>
        </div>
      </Drawer>
      ...
      if (err) {
        ...
        const filteredStations = result.forecasts.station.filter((station) => {
          ...
          station.forecast = station.forecast.filter((singleForecast) => {
            return (
              moment(singleForecast.ftime, 'YYYY-MM-DD HH:mm:ss') >= currentTime
            )
          })
